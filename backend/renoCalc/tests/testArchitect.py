...
from renoCalc.models import Contractor, Architect
...
class ArchitectTestCases(TestCase):
    ...
    def testArchitectWithContractorMade(self):
        ...
        ArchitectServiceWithAllFields = Architect.objects.create(
            contractor=self.contractorWithAllFields,
            description="testing",
            commission=commission_data,
        )

        self.assertTrue(Architect.objects.filter(description="testing").exists())

    def testArchitectWithoutNecessaryField(self):
        with self.assertRaises(IntegrityError):
            ArchitectServiceWithSomeFields = Architect.objects.create(
                contractor=self.contractorWithAllFields,
                description="test",
            )
            self.assertTrue(Architect.objects.filter(description="test").exists())
