...
class MyViewTestCase(TestCase):
    ...
    def test_my_view(self):
        url = reverse('saveUserConfiguration', kwargs={'userID': self.userWithAllFields.id})
        response = self.client.post(url, data=self.json_data, content_type='application/json')

        self.assertEqual(response.status_code, 200)

        dbUserConfigurationInstance = UserHomeAndRenovationConfigurationModel.objects.filter(user=self.userWithAllFields)
